...
/**
 * Custom validator for tax code validation
 * Validates format: 10 digits or 10 digits + "-" + 3 digits
 * @param title
 * @returns {ValidatorFn} The validator function
 */
export function inputTaxCode14Validator(title?: string): ValidatorFn {
  return (control: AbstractControl): ValidationErrors | null => {
    ...
    if (!/^[0-9-]+$/.test(value)) {
      return {
        invalidFormat: { message: `${title || 'Dữ liệu'} sai định dạng` },
      };
    }

    // Check length (must be 10 or 14 characters)
    if (value.length !== 10 && value.length !== 14) {
      return {
        invalidLength: { message: `${title || 'Dữ liệu'} sai định dạng` },
      };
    }

    // Check format for 14 characters: 10 digits + "-" + 3 digits
    if (value.length === 14) {
    ...
    // Check format for 10 characters: must be all digits
    if (value.length === 10) {
      if (!/^\d{10}$/.test(value)) {
        return {
          invalidFormat: {
...
@Component({
  selector: 'input-tax-code-14',
  templateUrl: './input-tax-code-14.component.html',
  styleUrls: ['./input-tax-code-14.component.css'],
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => InputTaxCode14Component),
      multi: true,
    },
  ],
  standalone: true,
...
export class InputTaxCode14Component
  implements ControlValueAccessor, OnInit, OnDestroy
{
  @Input() label: string = 'Mã số thuế';
  ...
  onInputChange(event: any): void {
    ...
    // Filter out invalid characters, keep only digits and hyphens
    value = value.replace(/[^0-9-]/g, '').substring(0, 14);
  ...
  private getDefaultErrorMessage(errorKey: string, errorValue: any): string {
    switch (errorKey) {
      ...
      case 'invalidLength':
        return errorValue.message;
      default:
        return 'Mã số thuế sai định dạng';
    }
  }
}
