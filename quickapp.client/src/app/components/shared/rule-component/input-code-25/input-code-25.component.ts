 ...
 */
export function inputCode25Validator(title?: string): ValidatorFn {
  return (control: AbstractControl): ValidationErrors | null => {
    ...
    if (value.length > 25) {
      return {
        maxLength: {
          message: `${title || 'Dữ liệu'} không được vượt quá 25 ký tự`,
        },
      };
    }
...
@Component({
  selector: 'input-code-25',
  templateUrl: './input-code-25.component.html',
  styleUrls: ['./input-code-25.component.css'],
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => InputCode25Component),
      multi: true,
    },
  ],
  standalone: true,
...
export class InputCode25Component
  implements ControlValueAccessor, OnInit, OnDestroy
{
  ...
  onInputChange(event: any): void {
    ...
    value = value.replace(/[^a-zA-Z0-9.-]/g, '').substring(0, 25);
