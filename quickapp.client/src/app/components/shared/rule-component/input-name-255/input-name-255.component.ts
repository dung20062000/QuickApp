 ...
 */
export function inputName255Validator(title?: string): ValidatorFn {
  return (control: AbstractControl): ValidationErrors | null => {
    ...
    if (value.length > 255) {
      return {
        nameMaxLength: { message: `${title || 'Dữ liệu'} sai định dạng` },
      };
    }
...
@Component({
  selector: 'input-name-255',
  templateUrl: './input-name-255.component.html',
  styleUrls: ['./input-name-255.component.css'],
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => InputName255Component),
      multi: true,
    },
  ],
  standalone: true,
...
export class InputName255Component
